...
docker build -t g-hotstuff .

# Run the container
docker run -it --name g-hotstuff-dev g-hotstuff /bin/bash
```
...
g-hotstuff is written in Rust, but all benchmarking scripts are written in Python and run with [Fabric](http://www.fabfile.org/).
To deploy and benchmark a testbed of 4 nodes on your local machine, clone the the anonymous repo link of g-hotstuff and install the python dependencies:

```
$ cd g-hotstuff/benchmark
...
The file `settings.json` located in `g-hotstuff/benchmark` contains all the configuration parameters of the testbed to deploy.


**4. Create a testbed**

The AWS instances are orchestrated with [Fabric](http://www.fabfile.org/) from the file `fabfile.py` (located in `g-hotstuff/benchmark`) you can list all possible commands as follows:
