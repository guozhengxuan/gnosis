...
class LogParser:
    ...
    def _parse_nodes(self, log):
        ...
        tmp_p = findall(r'\[(.*Z) .* Created B(\d+)\(([^ ]+)\) round (\d+) tag (\d+)', log)
        tmp = [(d, self._to_posix(t)) for t, _, d, _, _ in tmp_p]
        proposals = self._merge_results([tmp])

        tmp = [(d+"="+h+"="+g+"="+r,self._to_posix(t)) for t, h, d, r, g in tmp_p]
        h_proposals = self._merge_results([tmp])

        tmp_c = findall(r'\[(.*Z) .* Committed B(\d+)\(([^ ]+)\) round (\d+) tag (\d+)', log)
        tmp = [(d, self._to_posix(t)) for t, _, d, _, _ in tmp_c]
        commits = self._merge_results([tmp])

        tmp = [(d+"="+h+"="+g+"="+r,self._to_posix(t)) for t, h, d, r, g in tmp_c]
