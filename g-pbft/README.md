...
docker build -t g-pbft .

# Run the container
docker run -it --name g-pbft-dev g-pbft /bin/bash
```
...
g-pbft is written in Rust, but all benchmarking scripts are written in Python and run with [Fabric](http://www.fabfile.org/).
To deploy and benchmark a testbed of 4 nodes on your local machine, clone the the anonymous repo link of g-pbft and install the python dependencies:
...
The file `settings.json` located in `g-pbft/benchmark` contains all the configuration parameters of the testbed to deploy.


**4. Create a testbed**

The AWS instances are orchestrated with [Fabric](http://www.fabfile.org/) from the file `fabfile.py` (located in `g-pbft/benchmark`) you can list all possible commands as follows:
